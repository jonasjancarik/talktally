...
function FormatTime({seconds, includeMilliseconds = true, secondsPadding = 2, millisecondsDecimals = 2}) {
    if (seconds < 0) {
    ...
    let formattedTime = `${minutes.toString().padStart(secondsPadding, '0')}:${remainingSeconds.toString().padStart(secondsPadding, '0')}`;
    ...
    if (includeMilliseconds) {
        ...
        const millisecondsString = milliseconds.toString().padStart(millisecondsDecimals, '0');
